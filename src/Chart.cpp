...
void Signal::Edit(float* buf, int start, int size) {
	...
	const float y_high = y_zero - m_graph_region.height + 1;

	for (int i = 0, s = start; i < size; ++i, ++s) {
		m_curve[s].position.y = y_zero - (buf[i] / y_high) * m_graph_region.height + 1;
	}
}
...
Chart::Chart(int x, int y, int w, int h, int num_of_points, std::string const& title,
	...
	m_num_of_points(num_of_points), m_background(sf::Vector2f(w, h)),
...
const sf::FloatRect Chart::GetGraphRegion() const {
	return m_chart_rect;
}

void Chart::SetMaxVal(float max_val) {
	m_max_val = max_val;
}
